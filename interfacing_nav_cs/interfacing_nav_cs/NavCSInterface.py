...
NAV_JETSON_STATS_TOPIC = "/NAV/jetson_stats"
...
class NavCSInterface(Node):
    ...
    def __init__(self):
        ...
        # ====================================================================
        # Jetson Stats Subscriber
        # ====================================================================
        self.nav_jetson_stats_sub = self.create_subscription(
            String,
            NAV_JETSON_STATS_TOPIC,
            self.handle_nav_jetson_stats,
            qos_profile=self.qos_profile
        )
    ...
    # ========================================================================
    # Jetson Stats Subscription callback
    # ========================================================================
    def handle_nav_jetson_stats(self, msg: String):
        try:
            stats = json.loads(msg.data) if isinstance(msg.data, str) else {}
            if not isinstance(stats, dict):
                return

            # Normalize expected keys for CS parser
            self.nav_full_state["hardware"]["stats_nav"] = {
                "utilization_cpus": stats.get("utilization_cpus", [0.0] * 8),
                "ram": stats.get("ram", 0.0),
                "load_gpu": stats.get("load_gpu", 0.0),
                "fan_rpm": stats.get("fan_rpm", 0.0),
                "power_tot": stats.get("power_tot", 0.0),
                "temp_cpu": stats.get("temp_cpu", 0.0),
                "temp_gpu": stats.get("temp_gpu", 0.0),
            }
        except Exception as e:
            self.get_logger().warn(f"Failed to parse /NAV/jetson_stats: {e}")
