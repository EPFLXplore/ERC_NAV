...
RoverSlowNormalKinematicModel::RoverSlowNormalKinematicModel():
...
void RoverSlowNormalKinematicModel::init(motors_obj motors_position, _Float64 wheels_angle)
{
...
motors_obj RoverSlowNormalKinematicModel::run(motors_obj motors_position, _Float64 v_x, _Float64 v_y, _Float64 omega_z)
{
...
void RoverSlowNormalKinematicModel::rotation_translation(wheels_normal_kinematic_cmds motors_commands)
{
...
bool RoverSlowNormalKinematicModel::check_steering_position_for_translation(motors_obj current_motors_position) const 
{
    int wheel_positioned_for_translation = 0;

    for (auto motor = FRONT_LEFT; motor <= BACK_LEFT; motor++)
    {
        if (std::abs(current_motors_position.steer[motor]) < 1000)
...
bool RoverSlowNormalKinematicModel::check_steering_position_for_rotation(motors_obj current_motors_position) const 
{
    int wheel_positioned_for_rotation = 0;

    for (auto motor = 0; motor < NB_WHEELS; motor++) 
    {
        if (std::abs(std::abs(current_motors_position.steer[motor]) - wheels_angle_for_rotation) < 1000) 
        {
