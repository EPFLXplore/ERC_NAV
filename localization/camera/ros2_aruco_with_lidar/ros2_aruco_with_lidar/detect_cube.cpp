...
public:
    DetectCubeNode()
        ...
        min_inliers_ = 35; // at least 20 points to define a line
        max_lines_ = 3; // at least 2 lines to form a corner

        max_distance_from_aruco_ = 1.2; // rayon autour de l'ArUco
...
private:
    ...
    void detect_lignes(const sensor_msgs::msg::PointCloud2 &cloud_msg) {
        ...
        for (size_t aruco_idx = 0; aruco_idx < aruco_ids_.size(); ++aruco_idx) {
            ...
            float aruco_angle_deg = atan2(aruco_y, aruco_x) * 180.0f / M_PI;
            ...
            for (const auto& pt : full_cloud->points) {
                float dx = pt.x - aruco_x;
                float dy = pt.y - aruco_y;
                float dz = pt.z - aruco_z;
                ...
                double angle_diff = angDeltaDeg(pt_angle_deg, static_cast<double>(aruco_angle_deg) + 100.0);
                
                if (dist < max_distance_from_aruco_ && angle_diff < static_cast<double>(angular_tolerance_deg_)) {
