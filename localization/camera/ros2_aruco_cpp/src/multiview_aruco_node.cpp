...
private:
    ...
    void setup_camera_intrinsics() {
        // for each camera call the service /NAV/camera_info_<i> to get the intrinsics and fill cam_[i].intrinsic and cam_[i].distortion
        // the service will return a custom message containing the intrinsics and distortion in a format that can be easily converted to cv::Mat. The service will be implemented in the camera node and will read the calibration data from a json file that is generated by the calibration process. The json file will contain the intrinsics and distortion for each camera, indexed by serial number. The camera node will read the serial number from the parameters and will use it to respond to the service calls.
        for (int i = 0; i < NUM_CAMERAS; ++i) {
            // you need the came i==0 to be the one with /NAV/feed_camera_nav_0
            RCLCPP_INFO(get_logger(), "Requesting intrinsics");
            auto client = this->create_client<custom_msg::srv::CameraParams>("/NAV/camera_info_" + std::to_string(i));
            auto req = std::make_shared<custom_msg::srv::CameraParams::Request>();
            auto future = client->async_send_request(req);
            if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), future) !=
                rclcpp::FutureReturnCode::SUCCESS) {
                RCLCPP_ERROR(get_logger(), "Failed to call service for camera %d intrinsics", i);
                return;
            }
            ...
            RCLCPP_INFO(get_logger(), "Calibration fetched");
        }
    }
