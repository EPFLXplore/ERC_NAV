...
void ScanMatcherComponent::initializePubSub()
{
  ...
  initial_pose_sub_ =
    ...
    "/lio_sam/initial_pose", rclcpp::QoS(10), initial_pose_callback);

  odom_sub_ =
    create_subscription<nav_msgs::msg::Odometry>(
    "/lio_sam/preintegrated_odom", rclcpp::SensorDataQoS(), odom_callback);
  ...
  pose_pub_ = create_publisher<geometry_msgs::msg::PoseStamped>(
    "/lio_sam/current_pose",
    rclcpp::QoS(rclcpp::KeepLast(1)).reliable());
  map_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>(
    "/lio_sam/map", rclcpp::QoS(
      rclcpp::KeepLast(
        1)).reliable());
  map_array_pub_ =
    create_publisher<lidarslam_msgs::msg::MapArray>(
    "/lio_sam/map_array", rclcpp::QoS(
      rclcpp::KeepLast(
        1)).reliable());
  path_pub_ = create_publisher<nav_msgs::msg::Path>("/lio_sam/path", 1);
  odom_pub_ = create_publisher<nav_msgs::msg::Odometry>(
    "/lio_sam/odom", rclcpp::QoS(
      rclcpp::KeepLast(
        1)).reliable());

  destroy_sub_ = create_subscription<std_msgs::msg::String>("ROVER/NAV_status", rclcpp::QoS(rclcpp::KeepLast(5)), std::bind(&ScanMatcherComponent::destroy_callback, this,
      std::placeholders::_1));
}
  ...
  const Eigen::Matrix4f final_transformation, const rclcpp::Time stamp)
{
  ...
  corrent_pose_stamped_.header.stamp.sec = 0;
  corrent_pose_stamped_.header.stamp.nanosec = 0;
...
void ScanMatcherComponent::destroy_callback(const std_msgs::msg::String::SharedPtr msg)
{
  RCLCPP_INFO(get_logger(), "Destroying the scanmatcher component node.");
  rclcpp::shutdown();
