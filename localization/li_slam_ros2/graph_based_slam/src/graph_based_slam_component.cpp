...
void GraphBasedSlamComponent::initializePubSub()
{
  ...
  map_array_sub_ =
    ...
    "/lio_sam/map_array", rclcpp::QoS(rclcpp::KeepLast(1)).reliable(), map_array_callback);
  ...
  modified_map_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>(
    "/lio_sam/modified_map",
    rclcpp::QoS(10));

  modified_map_array_pub_ = create_publisher<lidarslam_msgs::msg::MapArray>(
    "/lio_sam/modified_map_array", rclcpp::QoS(10));

  modified_path_pub_ = create_publisher<nav_msgs::msg::Path>(
    "/lio_sam/modified_path",
    rclcpp::QoS(10));

  destroy_sub_ = create_subscription<std_msgs::msg::String>(
    "ROVER/NAV_status",
    rclcpp::QoS(rclcpp::KeepLast(5)), std::bind(&GraphBasedSlamComponent::destroy_callback, this,
      std::placeholders::_1));
...
void GraphBasedSlamComponent::destroy_callback(const std_msgs::msg::String::SharedPtr msg)
{
  if (msg->data == "abort") 
  {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Destroying graph-based slam node...");
    rclcpp::shutdown();
  }
}
...
// node -> create_subscription<std_msgs::msg::String>("ROVER/NAV_status",
