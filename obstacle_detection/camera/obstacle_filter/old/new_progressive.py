...
def getPC(depth_array, conf_array, gridshape, threshold, \
          max_depth = 3000, min_depth = 100, num_iter = 20, thresh_dist = 20, num_inliers = 200, \
            how_to_replace = 'random', ground_vector = torch.tensor([0, 1, 0]), filter = True):
    # before 1280 720
    # after 256 144
    skip_points = 5
    ...
    best_planes, valid_planes = ransac(array, num_iter, thresh_dist, num_inliers, num_random_pts=3, device='cpu') # grid_x x grid_y x 4
...
print(conf_array_.dtype)

gridshape = (8, 8)

orig_points = getPC(depth_array_, conf_array_, gridshape, threshold, filter=False)

for i in range(orig_points.shape[1]):
    for j in range(orig_points.shape[2]):
        pc = o3d.geometry.PointCloud()
        pc.points = o3d.utility.Vector3dVector(orig_points[:, i, j, :].T)
        color = np.array([0.5, 0.5, 0.5])
        pc.colors = o3d.utility.Vector3dVector(np.repeat(color.reshape(1, 3), orig_points.shape[3], axis=0))
        visualization_window.add_geometry(pc)


# masks = masks(depth_array_.shape[0], depth_array_.shape[1], gridshape)
t1 = time.time()
orig_points = getPC(depth_array_, conf_array_, gridshape, threshold, filter=True)
t2 = time.time()
print('time taken: ', t2 - t1)

for i in range(orig_points.shape[1]):
    pc = o3d.geometry.PointCloud()
    pc.points = o3d.utility.Vector3dVector(orig_points[:, i, :].T)
    color = np.random.rand(3)
    pc.colors = o3d.utility.Vector3dVector(np.repeat(color.reshape(1, 3), orig_points.shape[2], axis=0))
    visualization_window.add_geometry(pc)

# subgrid_size = (orig_points.shape[1] // gridshape[0], orig_points.shape[2] // gridshape[1])

# # grid_height = (orig_points.shape[1] // gridshape[0])
# # grid_width = (orig_points.shape[2] // gridshape[1])
# # print(orig_points.shape)

# subimages = torch.split(orig_points, subgrid_size[0], dim=1)
# print(len(subimages), subimages[0].shape)
# subimages = [torch.split(subimage, subgrid_size[1], dim=2) for subimage in subimages]

# subimages_tensor = torch.stack([torch.stack(subimage_row, dim=1) for subimage_row in subimages], dim=1)
# print(subimages_tensor.shape)

# orig_points = orig_points.reshape(3, gridshape[0], gridshape[1], grid_height, grid_width)
#orig_points = orig_points < 3000



# for i in range(subimages_tensor.shape[1]):
#     for j in range(subimages_tensor.shape[2]):
#         pc = o3d.geometry.PointCloud()
#         new_points = subimages_tensor[:, i, j, :, :].reshape(3, -1).T

#         new_points = new_points[new_points[:, 2] < 3000, :]
#         pc.points = o3d.utility.Vector3dVector(new_points)
#         color = np.random.rand(3)
#         pc.colors = o3d.utility.Vector3dVector(np.repeat(color.reshape(1, 3), new_points.shape[0], axis=0))
#         visualization_window.add_geometry(pc)


# pc_orig = o3d.geometry.PointCloud()
# pc_orig.points = o3d.utility.Vector3dVector(orig_points)

# color = np.array([[0.5, 0.5, 0.5]])
# pc_orig.colors = o3d.utility.Vector3dVector(np.repeat(color, orig_points.shape[0], axis=0))
# visualization_window.add_geometry(pc_orig)
