...
OusterDriver::OusterDriver(std::unique_ptr<SensorInterface> sensor,
  ...
  this->declare_parameter("timestamp_mode",
                          std::string("TIME_FROM_INTERNAL_OSC"));
  ...
  destroy_sub_ = this->create_subscription<std_msgs::msg::String>(
      "/ROVER/NAV_status", rclcpp::QoS(rclcpp::KeepLast(5)),
      std::bind(&OusterDriver::destroy_callback, this, _1));
}
...
void OusterDriver::onConfigure() {
  ...
  RCLCPP_INFO(this->get_logger(), "Connecting to sensor at %s.",
              lidar_config.lidar_ip.c_str());
  RCLCPP_INFO(this->get_logger(), "Sending data from sensor to %s.",
  ...
  _metadata_srv = this->create_service<ouster_msgs::srv::GetMetadata>(
      "~/get_metadata",
      std::bind(&OusterDriver::getMetadata, this, _1, _2, _3));
  ...
  _full_rotation_accumulator =
      std::make_shared<sensor::FullRotationAccumulator>(
          _sensor->getMetadata(), _sensor->getPacketFormat());

  if (_use_system_default_qos) {
    RCLCPP_INFO(this->get_logger(),
                "Using system defaults QoS for sensor data");
    _data_processors = ros2_ouster::createProcessors(
        shared_from_this(), _sensor->getMetadata(), _imu_data_frame,
        _laser_data_frame, rclcpp::SystemDefaultsQoS(),
        _sensor->getPacketFormat(), _full_rotation_accumulator, _proc_mask);
  } else {
    _data_processors = ros2_ouster::createProcessors(
        shared_from_this(), _sensor->getMetadata(), _imu_data_frame,
        _laser_data_frame, rclcpp::SensorDataQoS(), _sensor->getPacketFormat(),
        _full_rotation_accumulator, _proc_mask);
  }

  _tf_b =
      std::make_unique<tf2_ros::StaticTransformBroadcaster>(shared_from_this());
...
void OusterDriver::broadcastStaticTransforms(
  ...
  if (_tf_b) {
    ...
    transforms.push_back(toMsg(mdata.imu_to_sensor_transform,
                               _laser_sensor_frame, _imu_data_frame,
                               this->now()));
...
void OusterDriver::processData() {
  try {
    ...
    RCLCPP_DEBUG(this->get_logger(), "Packet with state: %s",
    ...
    if (_lidar_packet_data) {
      ...
      key_its = _data_processors.equal_range(
          ouster::sensor::client_state::LIDAR_DATA);
...
void OusterDriver::destroy_callback(const std_msgs::msg::String::SharedPtr msg) {
  if (msg->data == "abort") {
    RCLCPP_INFO(this->get_logger(), "Destroying ouster driver node");
    this->onShutdown();
    rclcpp::shutdown();
  }
