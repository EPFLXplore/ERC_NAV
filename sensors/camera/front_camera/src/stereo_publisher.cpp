...
int main(int argc, char** argv) {
    ...
    auto leftQueue = device.getOutputQueue("left", 10, false); // we only keep the newest
    auto rightQueue = device.getOutputQueue("right", 10, false);
    std::shared_ptr<dai::DataOutputQueue> stereoQueue;
    if(enableDepth) {
        stereoQueue = device.getOutputQueue("depth", 10, false);
    }
    ...
    if(mode == "depth") {
        dai::rosBridge::BridgePublisher<sensor_msgs::msg::Image, dai::ImgFrame> depthPublish(
            stereoQueue,
            node,
            std::string("stereo/depth"), // used to be stereo/depth
